...
<input type="text" name="no" value="${upReq.articleNumber }" hidden/>
<p>
번호 <br /> ${upReq.articleNumber }
</p>
<p>
제목 <br /> <input type="text" name = "title" value="${upReq.title }" />
<c:if test="${errors.title }">제목을 입력하시오</c:if>
</p>
<p>
내용 <br /> 
<textarea name="content" id="" cols="30" rows="5">${upReq.content }</textarea>
</p>
	...
	<section class="container">
		<form action="modify.do" method="post">
			<div class="WritingWrap">
				...
				<input type="text" name="no" value="${upReq.articleNumber }" hidden/>
					<h3>게시글 수정</h3>
					<button class="btn btn-succes">수정</button>
				</div>
			</div>
				...
				<div class="WritingEditor">
					<div class="ArticleWritingTitle">
						<div class="FlexableTextArea">
							<textarea placeholder="제목을 입력하세요." class="textarea_input" style="height: 40px;" name="title">${upReq.title }</textarea>
							<small><c:if test="${errors.title }">제목을 입력하세요</c:if></small>
						</div>
					</div>
					<div class="CafeEditor">
						<div class="FlexableTextArea">
							<textarea  placeholder="내용을 입력하세요"  class="textarea_input" name="content" style="height:480px;" >${upReq.content }</textarea>
						</div>
					</div>
				</div>
			</div>
